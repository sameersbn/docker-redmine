...
Current Version: **sameersbn/redmine:6.0.10-1**
...
```bash
docker pull sameersbn/redmine:6.0.10-1
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:6.0.10-1 app:backup:create
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:6.0.10-1 app:backup:restore
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:6.0.10-1 app:backup:restore BACKUP=1417624827_redmine_backup.tar
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:6.0.10-1 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:6.0.10-1 app:rake redmine:attachments:prune
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:6.0.10-1
```
...
- Commands to run to update image to next redmine release, examples are from 6.0.10 to 6.0.10

```bash
sed -i 's/6.0.10/6.0.10/g' VERSION README.md docker-compose* Dockerfile
...
- Select tag 6.0.10 and set release title to 6.0.10
- Publish release
