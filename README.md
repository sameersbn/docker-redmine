...
```bash
docker pull quay.io/sameersbn/redmine:2.6.7-2
```
...
docker run --name=redmine -d \
  ...
  quay.io/sameersbn/redmine:2.6.7-2
```
...
```bash
docker run --name=redmine -d [OPTIONS] quay.io/sameersbn/redmine:2.6.7-2
```
...
docker run --name=redmine -d [OPTIONS] \
  quay.io/sameersbn/redmine:2.6.7-2 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  quay.io/sameersbn/redmine:2.6.7-2 app:rake redmine:attachments:prune
```
