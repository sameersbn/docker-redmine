...
```bash
docker pull quay.io/sameersbn/redmine:3.0.5-3
```
...
docker run --name=redmine -d \
  ...
  quay.io/sameersbn/redmine:3.0.5-3
```
...
```bash
docker run --name=redmine -d [OPTIONS] quay.io/sameersbn/redmine:3.0.5-3
```
...
docker run --name=redmine -d [OPTIONS] \
  quay.io/sameersbn/redmine:3.0.5-3 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  quay.io/sameersbn/redmine:3.0.5-3 app:rake redmine:attachments:prune
```
