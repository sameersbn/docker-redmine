...
```bash
docker pull sameersbn/redmine:2.6.9
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:2.6.9
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:2.6.9 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:2.6.9 app:rake redmine:attachments:prune
```
