...
Current Version: **sameersbn/redmine:4.1.1-5**
...
```bash
docker pull sameersbn/redmine:4.1.1-5
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:4.1.1-5 app:backup:create
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:4.1.1-5 app:backup:restore
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:4.1.1-5 app:backup:restore BACKUP=1417624827_redmine_backup.tar
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:4.1.1-5 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:4.1.1-5 app:rake redmine:attachments:prune
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:4.1.1-5
```
...
* Commands to run to update image to next redmine release, examples are from 4.1.1-5 to 4.1.1

```bash
sed -i 's/4.1.1-5/4.1.1-6/g' VERSION README.md docker-compose-mysql.yml docker-compose-ssl.yml Dockerfile docker-compose.yml
...
* Select tag 4.1.1-5 and set release title to 4.1.1-5
* Publish release
