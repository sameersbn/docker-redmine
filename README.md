...
Current Version: **sameersbn/redmine:4.1.1-7**
...
```bash
docker pull sameersbn/redmine:4.1.1-7
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:4.1.1-7 app:backup:create
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:4.1.1-7 app:backup:restore
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:4.1.1-7 app:backup:restore BACKUP=1417624827_redmine_backup.tar
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:4.1.1-7 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:4.1.1-7 app:rake redmine:attachments:prune
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:4.1.1-7
```
...
* Commands to run to update image to next redmine release, examples are from 4.1.1-7 to 4.1.1

```bash
sed -i 's/4.1.1-7/4.1.1-7/g' VERSION README.md docker-compose-mysql.yml docker-compose-ssl.yml Dockerfile docker-compose.yml
...
* Select tag 4.1.1-7 and set release title to 4.1.1-7
* Publish release
