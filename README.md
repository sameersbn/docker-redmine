...
```bash
docker pull sameersbn/redmine:2.6.4
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:2.6.4
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:2.6.4 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:2.6.4 app:rake redmine:attachments:prune
```
