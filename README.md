...
```bash
docker pull sameersbn/redmine:2.6.7
```
...
*P.S. The receiving emails feature is only available since versions `2.6.7`, `3.0.4-2` and `3.1.0-2`. Refer the [Changelog](Changelog.md) for details.*
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:2.6.7
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:2.6.7 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:2.6.7 app:rake redmine:attachments:prune
```
