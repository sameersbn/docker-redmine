...
Current Version: **sameersbn/redmine:6.1.1**
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:6.1.1 app:backup:restore BACKUP=1417624827_redmine_backup.tar
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:6.1.1 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:6.1.1 app:rake redmine:attachments:prune
```
...
- Commands to run to update image to next redmine release, examples are from 6.1.1 to 6.1.1

```bash
sed -i 's/6.1.1/6.1.1/g' VERSION README.md docker-compose* Dockerfile
...
- Select tag 6.1.1 and set release title to 6.1.1
- Publish release
