...
Current Version: **sameersbn/redmine:7.0.0-1**
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:7.0.0-1 app:backup:restore BACKUP=1417624827_redmine_backup.tar
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:7.0.0-1 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:7.0.0-1 app:rake redmine:attachments:prune
```
