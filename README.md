...
Current Version: **sameersbn/redmine:3.3.10-1**
...
```bash
docker pull sameersbn/redmine:3.3.10-1
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:3.3.10-1 app:backup:create
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:3.3.10-1 app:backup:restore
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:3.3.10-1 app:backup:restore BACKUP=1417624827_redmine_backup.tar
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:3.3.10-1 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:3.3.10-1 app:rake redmine:attachments:prune
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:3.3.10-1
```
...
* Commands to run to update image to next redmine release, examples are from 4.0.5 to 3.3.10-1

```bash
sed -i 's/3.3.9/3.3.10-1/' VERSION README.md docker-compose-mysql.yml Dockerfile docker-compose.yml
...
* Select tag 3.3.10-1 and set release title to 3.3.10
* Publish release
