...
Current Version: **sameersbn/redmine:5.0.14**
...
```bash
docker pull sameersbn/redmine:5.0.14
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:5.0.14 app:backup:create
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:5.0.14 app:backup:restore
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:5.0.14 app:backup:restore BACKUP=1417624827_redmine_backup.tar
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:5.0.14 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:5.0.14 app:rake redmine:attachments:prune
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:5.0.14
```
...
- Commands to run to update image to next redmine release, examples are from 5.0.14 to 5.0.14

```bash
sed -i 's/5.0.14/5.0.14/g' VERSION README.md docker-compose-memcached.yml docker-compose-mysql.yml docker-compose-ssl.yml docker-compose-sqlite3.yml docker-compose-mariadb.yml Dockerfile docker-compose.yml
...
- Select tag 5.0.14 and set release title to 5.0.14
- Publish release
