...
```bash
docker pull sameersbn/redmine:2.6.7-3
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:2.6.7-3
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:2.6.7-3 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:2.6.7-3 app:rake redmine:attachments:prune
```
