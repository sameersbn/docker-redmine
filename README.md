...
Current Version: **sameersbn/redmine:5.0.4**
...
```bash
docker pull sameersbn/redmine:5.0.4
```
...
_Please Note: this [plugin install script](https://gist.github.com/sameersbn/dd24dfdd13bc5.0.41a5) is not maintained and you would need to fix it if required (especially broken links)_
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:5.0.4 app:backup:create
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:5.0.4 app:backup:restore
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:5.0.4 app:backup:restore BACKUP=1417624827_redmine_backup.tar
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:5.0.4 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:5.0.4 app:rake redmine:attachments:prune
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:5.0.4
```
...
- Commands to run to update image to next redmine release, examples are from 5.0.4 to 5.0.4

```bash
sed -i 's/5.0.4/5.0.4/g' VERSION README.md docker-compose-memcached.yml docker-compose-mysql.yml docker-compose-ssl.yml docker-compose-sqlite3.yml docker-compose-mariadb.yml Dockerfile docker-compose.yml
...
- Select tag 5.0.4 and set release title to 5.0.4
- Publish release
