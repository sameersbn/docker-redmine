...
```bash
docker pull sameersbn/redmine:2.6.6-2
```
...
*P.S. The receiving emails feature is only available in versions greater than `2.6.6`, `3.0.4` and `3.1.0-1`. Refer the [Changelog](Changelog.md) for details.*
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:2.6.6-2
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:2.6.6-2 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:2.6.6-2 app:rake redmine:attachments:prune
```
