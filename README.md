...
```bash
docker pull sameersbn/redmine:3.1.3
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:3.1.3
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:3.1.3 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:3.1.3 app:rake redmine:attachments:prune
```
