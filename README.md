...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:5.1.3 app:backup:create
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:5.1.3 app:backup:restore
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:5.1.3 app:backup:restore BACKUP=1417624827_redmine_backup.tar
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:5.1.3 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:5.1.3 app:rake redmine:attachments:prune
```
...
- Commands to run to update image to next redmine release, examples are from 5.0.4 to 5.1.3

```bash
sed -i 's/5.1.3/5.1.3/g' VERSION README.md docker-compose-memcached.yml docker-compose-mysql.yml docker-compose-ssl.yml docker-compose-sqlite3.yml docker-compose-mariadb.yml Dockerfile docker-compose.yml
...
- Select tag 5.1.3 and set release title to 5.1.3
- Publish release
