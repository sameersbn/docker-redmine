...
Current Version: **sameersbn/redmine:4.1.3**
...
```bash
docker pull sameersbn/redmine:4.1.3
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:4.1.3 app:backup:create
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:4.1.3 app:backup:restore
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:4.1.3 app:backup:restore BACKUP=1417624827_redmine_backup.tar
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:4.1.3 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:4.1.3 app:rake redmine:attachments:prune
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:4.1.3
```
...
* Commands to run to update image to next redmine release, examples are from 4.1.3 to 4.1.3

```bash
sed -i 's/4.1.3/4.1.3/g' VERSION README.md docker-compose-mysql.yml docker-compose-ssl.yml docker-compose-sqlite3.yml Dockerfile docker-compose.yml
...
* Select tag 4.1.3 and set release title to 4.1.3
* Publish release
