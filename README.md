...
Current Version: **sameersbn/redmine:7.0.0**
...
```bash
docker pull sameersbn/redmine:7.0.0
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:7.0.0 app:backup:create
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:7.0.0 app:backup:restore
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:7.0.0 app:backup:restore BACKUP=1417624827_redmine_backup.tar
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:7.0.0 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:7.0.0 app:rake redmine:attachments:prune
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:7.0.0
```
...
- Commands to run to update image to next redmine release, examples are from 7.0.0 to 7.0.0

```bash
sed -i 's/7.0.0/7.0.0/g' VERSION README.md docker-compose* Dockerfile
...
- Select tag 7.0.0 and set release title to 7.0.0
- Publish release
