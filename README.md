...
```bash
docker pull sameersbn/redmine:3.0.7
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:3.0.7
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:3.0.7 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:3.0.7 app:rake redmine:attachments:prune
```
