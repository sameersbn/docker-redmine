...
Current Version: **sameersbn/redmine:3.1.4-1**
...
```bash
docker pull sameersbn/redmine:3.1.4-1
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:3.1.4-1 app:backup:create
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:3.1.4-1 app:backup:restore
```
...
```bash
docker run --name redmine -it --rm [OPTIONS] \
  sameersbn/redmine:3.1.4-1 app:backup:restore BACKUP=1417624827_redmine_backup.tar
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:3.1.4-1 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:3.1.4-1 app:rake redmine:attachments:prune
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:3.1.4-1
```
