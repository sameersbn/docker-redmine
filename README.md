...
```bash
docker pull sameersbn/redmine:3.0.5
```
...
*P.S. The receiving emails feature is only available since versions `2.6.6-2`, `3.0.5` and `3.1.0-2`. Refer the [Changelog](Changelog.md) for details.*
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:3.0.5
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:3.0.5 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:3.0.5 app:rake redmine:attachments:prune
```
