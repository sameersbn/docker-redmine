...
*P.S. The receiving emails feature is only available since versions `2.6.6-2`, `3.0.4-2` and `3.1.1`. Refer the [Changelog](Changelog.md) for details.*
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:3.1.1
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:3.1.1 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:3.1.1 app:rake redmine:attachments:prune
```
