...
```bash
docker pull sameersbn/redmine:3.0.5-1
```
...
```bash
docker run --name=redmine -d [OPTIONS] sameersbn/redmine:3.0.5-1
```
...
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:3.0.5-1 app:rake redmine:email:test[admin]
```
...
```bash
docker run --name=redmine -d [OPTIONS] \
  sameersbn/redmine:3.0.5-1 app:rake redmine:attachments:prune
```
